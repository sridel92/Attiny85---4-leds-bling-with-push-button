...
const int Switch  = 3;    // PB3 (pin 2 of ATtiny85    
volatile int s0   = 1;   // 
...
ISR (PCINT0_vect) {
 s0++;   // 1 interrupts on each press and release  : on Press button, s0 is 1 and then on release s0 is 2
 ChangeSwitch();
}
...
switch (s0) {  // you have an interrupt on press and another on release, so coounting case 1 + 2 for the first, case 3 + 4 for the second...
  case 1 ... 2:
  ...
  if (s0 == 9) { // restart the s0 at the beginning and going to first case 1 ... 2
    s0 = 0;
  }
  break;
  }
...
switch (s0) {
  ...
  case 1 ... 2: // ***** random 4 LEDS and 32 seconds delay *****
  ...
  case 5 ... 6: // *****  4 leds up and down - 4 seconds sleep *****
    for ( int y=0; y<5; y++ ) {
  ...
  case 7 ... 8: // ***** 2 + 2 leds - 4 seconds sleep *****
  for ( int y=0; y<5; y++ ) {
        digitalWrite(LedArray[0], HIGH);
        digitalWrite(LedArray[2], HIGH);
        WDDelay(1);  //0=16ms, 1=32ms, 2=64ms, 3=128ms, 4=250ms, 5=500ms  
